...
var _all_waves_signal_sent: bool = false
...
func _check_all_waves_spawned() -> void:
	if _all_waves_signal_sent:
		return

	for was_spawned in _spawned:
		if not was_spawned:
			return

	_all_waves_signal_sent = true
	all_waves_spawned.emit()
	print("WaveSpawner: wszystkie fale wypuszczone.")
